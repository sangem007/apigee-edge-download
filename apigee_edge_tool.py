...
apigee_mgmt_url="https://api.enterprise.apigee.com/v1/organizations/"

print("This tool Downloads All the Apigee Edge Data")
apigee_edge_org_name = input("Enter Apigee Edge Org Name: ")
apigee_edge_token = input("Enter Apigee Edge Org Token: ")

response,status_code=DownloadResources.get_resource("apis",apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token)
if status_code == 200:
    ...
    while user_choice !='quit':
        ...
        if user_choice == "all" or user_choice == "ts":
            apigee_edge_evn=DownloadResources.Envs(apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token)
            arr_env  = apigee_edge_evn.split(',')
            for env in arr_env:
                env = re.sub(r'[\[\]\"]','',env)
                env=env.strip();
                DownloadResources.Target_Servers(apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token,env)
        ...
        if user_choice == "all" or user_choice == "kvm":    
            ...
            for env in arr_env:
              env = re.sub(r'[\[\]\"]','',env)
              env=env.strip();
              DownloadResources.Kvms_Env_Level(apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token,env)
            print("All KVM's Downloaded")

        if user_choice == "all" or user_choice == "dev":
            DownloadResources.Developers(apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token)
            print("All Developers Downloaded")

        if user_choice == "all" or user_choice == "app":
            list_of_apps,status_code_apps=DownloadResources.get_resource("apps",apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token)
            ...
            for app in arr_apps:
            	...
            	app = re.sub(r'[\[\]\"\,\s*\}\{]','',app)
            	if app != '':
            		DownloadResources.Apps(apigee_mgmt_url,app,apigee_edge_org_name,apigee_edge_token)
            print("All Apps Downloaded")

        if user_choice == "all" or user_choice == "product":
            list_of_products,status_code_products=DownloadResources.get_resource("apiproducts",apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token)
            ...
            for product in arr_products:
            	...
            	product=product.strip()
            	if product !='':
            		DownloadResources.Products(apigee_mgmt_url,product,apigee_edge_org_name,apigee_edge_token)
            print("All Products Downloaded")

        if user_choice == "all" or user_choice == "proxy":
            list_of_proxies,status_code_proxies=DownloadResources.get_resource("apis",apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token)
            ...
            for proxy_names in arr_proxies:
                ...
                if proxy_names != '':
                    ...
                    DownloadResources.Proxies(apigee_mgmt_url,proxy_names,apigee_edge_org_name,apigee_edge_token)
            print("All Proxies Downloaded")
        
        if user_choice == "all" or user_choice == "sf":
            list_of_shared_flows,status_code_shared_flows=DownloadResources.get_resource("sharedflows",apigee_mgmt_url,apigee_edge_org_name,apigee_edge_token)
            ...
            for shared_flow in arr_shared_flows:
                ...
                if shared_flow != '':
                    ...
                    DownloadResources.Shared_Flows(apigee_mgmt_url,shared_flow,apigee_edge_org_name,apigee_edge_token)
